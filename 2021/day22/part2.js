...
function overwrittenVolume(a, cuboids) {
  return cuboids
    .map((b, i) => {
      const intersection = a.map((aRange, j) =>
        intersect2d(aRange, b.ranges[j])
      );
      if (intersection.includes(null)) return 0; // no overlapping volume
      return (
        volume(intersection) -
        overwrittenVolume(intersection, cuboids.slice(1 + i))
      );
    })
...
function countOn(cuboids) {
  ...
  for (let i = 0; i < cuboids.length; i++)
    if (cuboids[i].on)
      total +=
        volume(cuboids[i].ranges) -
        overwrittenVolume(cuboids[i].ranges, cuboids.slice(i + 1));
  return total;
}
...
function main() {
  console.time("speed");
  const result = part2(readLines(path.resolve(__dirname, filename)));
