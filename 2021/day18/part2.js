...
function implementation(lines = []) {
  ...
  for (let i = 0; i < lines.length; i++) {
    for (let j = 0; j < lines.length; j++) {
      ...
      const line1 = buildGraph(JSON.parse(lines[i]));
      const line2 = buildGraph(JSON.parse(lines[j]));

      let fish = line1.add(line2);

      while (fish.explode() || fish.split());

      let mag = magnitude(fish);

      if (mag > maxMag) {
        maxMag = Math.max(maxMag, mag);
        maxLines = [lines[i], lines[j]];
        reduced = str(fish);
      }
    }
  }

  console.log(maxLines[0]);
  console.log(maxLines[1]);
  console.log(reduced);
